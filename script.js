...
/* ================= STORAGE (Firestore, per signed-in student) =================
   CLOUD.data is an in-memory cache of the student's Firestore document.
   lsGet/lsSet read/write that cache; writes are debounced and pushed to
   Firestore in the background so the UI never has to wait on the network. */
const CLOUD = { uid: null, data: {}, saveTimer: null };
...
function applyStudentUI(){
  ...
  if(student.name){
    ...
    document.getElementById('studentChipText').innerHTML = `${student.name.split(' ')[0]} • ${student.cls}`;
  } else {
...
function confirmLogout(){
  if(confirm('Log out of your Google account? Your progress is saved and will be here next time you sign in.')){
    switchUser();
  }
}
...
async function switchUser(){
  ...
  student = {name:'', cls:''};
  CLOUD.uid = null; CLOUD.data = {}; // stop writing to the previous student's cloud doc
  editingProfile = false;
  applyStudentUI();
  ...
  gate.style.opacity = '1';
  gate.style.transform = 'scale(1)';
  // onAuthStateChanged will fire from signOut() and show the Google step again
}
...
function renderDashboard(){
  ...
  if(board.length===0){
    lbEl.innerHTML = `<div class="dash-empty">No attempts recorded on this device yet. Complete a test to appear on the leaderboard!</div>`;
  } else {
